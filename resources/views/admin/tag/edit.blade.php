    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        ...
        <li>
            <div class="flex items-center">
                ...
                <a href="{{ route('tag.index') }}" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">Tag</a>
            </div>
        </li>
        <li>
            <div class="flex items-center">
                ...
                <a href="{{ route('tag.show', $tag->id) }}" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2">{{$tag->name}}</a>
            </div>
        </li>
        <li aria-current="page">
            <div class="flex items-center">
                ...
                <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Cập nhật tag</span>
            </div>
        </li>
    </ol>
</nav>
...
<section class="bg-gray-50 dark:bg-gray-900 py-4 sm:py-5 mt-5">
    ...
    <h2 class=" mx-4 mb-4 text-xl font-bold text-blue-600">Cập nhật tag</h2>
    <form  method="POST" action="{{ route('tag.update', $tag->id) }}" class="px-4" enctype="multipart/form-data">
        ...
        <div class="flex flex-col gap-4">
            <div class="sm:col-span-2">
                ...
                <input type="text" name="id" id="id" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5" value="{{ $tag->id }}" disabled>
            </div>
            <div class="sm:col-span-2">
                <label for="slug" class="block mb-2 font-semibold text-gray-900">Tag slug</label>
                <input type="text" name="slug" id="slug" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5" value="{{ $tag->slug }}">
            </div>
            <div class="sm:col-span-2">
                <label for="name" class="block mb-2 font-semibold text-gray-900">Tag</label>
                <input type="text" name="name" id="name" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5" value="{{ $tag->name }}">
            </div>
        </div>
