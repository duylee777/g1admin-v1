    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        ...
        <li>
            <div class="flex items-center">
                ...
                <a href="{{ route('article.show', $article->id) }}" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">{{ $article->id }}</a>
            </div>
        </li>
        <li aria-current="page">
            <div class="flex items-center">
                ...
                <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Chỉnh sửa bài viết</span>
            </div>
        </li>
    </ol>
</nav>
...
<section class="bg-gray-50 dark:bg-gray-900 py-4 sm:py-5 mt-5">
    ...
    <h2 class=" mx-4 mb-4 text-xl font-bold text-yellow-600">Chỉnh sửa bài viết</h2>
    <form  method="POST" action="{{ route('article.update', $article->id) }}" class="px-4" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="flex flex-col gap-4">
            <div class="sm:col-span-2">
                <label for="id" class="block mb-2 font-semibold text-gray-900">ID</label>
                <input type="text" name="id" id="id" class="bg-gray-100 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5" value="{{ $article->id }}" disabled>
            </div>
            <div class="sm:col-span-2">
                <label for="title" class="block mb-2 font-semibold text-gray-900">Tiêu đề bài viết</label>
                <input type="text" name="title" id="title" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5" value="{{ $article->title }}" required="">
            </div>
            <div class="sm:col-span-2">
                <label for="slug" class="block mb-2 font-semibold text-gray-900">Slug</label>
                <input type="text" name="slug" id="slug" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5" value="{{ $article->slug }}" required="">
            </div>
            <div class="sm:col-span-2">
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="description" id="description" cols="" rows="3" class="ckeditor w-full focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow">{{ $article->description }}</textarea>
            </div>
            <div class="sm:col-span-2">
                <label for="detail" class="block mb-2 font-semibold text-gray-900">Nội dung chi tiết</label>
                <textarea name="detail" id="detail" cols="" rows="3" class="ckeditor w-full focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow">{{ $article->detail }}</textarea>
            </div>
            <div class="sm:col-span-2">
                <label for="image" class="block mb-2 font-semibold text-gray-900">Ảnh bìa của bài viết</label>
                <div id="article-image" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div class="flex items-center justify-center overflow-hidden bg-white p-4 rounded shadow h-60">
                        <img id="preview_image" class="w-full rounded-lg" src="{{asset('../storage/articles/'.$article->category_id.'/'.$article->image) }}" alt="Extra large avatar">
                    </div>
                </div>
